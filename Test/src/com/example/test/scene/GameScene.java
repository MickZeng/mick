...
import com.example.test.entity.GameGroup;
import com.example.test.entity.HelpLayer;
import com.example.test.res.Res;
import com.example.test.util.ConstantUtil;
import com.example.test.util.SharedUtil;
...
public class GameScene extends Scene {
	...
	/** The target score */
//	private ButtonSprite btnTargetScore;

	private AnimatedSprite targetScoreBg;
	private Text tTargetScore;

	private int[] targetScores = { 128, 256, 512, 1024, 2048 };
	...
	private void initView() {
		...
		targetScoreBg = new AnimatedSprite(0, currScoreBg.getY(),
				Res.GAME_SCORE_BG_NOW, getVertexBufferObjectManager());
		targetScoreBg.setRightPositionX(currScoreBg.getLeftX() - 20);
		this.attachChild(targetScoreBg);

		tTargetScore = new Text(0, targetScoreBg.getY() + 50,
				FontRes.getFont("target_score"), "2048", 4,
				getVertexBufferObjectManager());
		tTargetScore.setCentrePositionX(targetScoreBg.getCentreX());
		this.attachChild(tTargetScore);
	...
	private OnClickListener onClickListener = new OnClickListener() {
		...
		public void onClick(ButtonSprite pButtonSprite, float pTouchAreaLocalX,
			...
			// else if (pButtonSprite == btnTargetScore) {
			// /** change target score */
			// changedTargetScore();
			// }
	...
	private void changedTargetScore() {
		getActivity().runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				new AlertDialog.Builder(getActivity())
						.setTitle("Target Score")
						.setMessage("Is Change target score！")
	...
	private void init() {
		...
		int targetScore = targetScores[random];

//		tTargetScore.setText(targetScore + "");
//		tTargetScore.setCentrePositionX(targetScoreBg.getCentreX());

		mGameGroup.resetGame(targetScore);
	}
